...
class ParameterFileReader {
  ...
  def readForOtomoto(name: String, os: String): List[(String, String, BigInt, BigInt)] = {
    ...
    val parametersFile = os match {
      case x if x.startsWith("Windows") => scala.io.Source.fromFile(System.getProperty("user.dir") + s"\\conf\\$name")
      case x if x.startsWith("Linux") => scala.io.Source.fromFile(System.getProperty("user.dir") + s"/conf/$name")
      case _ => scala.io.Source.fromFile(System.getProperty("user.dir") + s"/conf/$name")
    }
